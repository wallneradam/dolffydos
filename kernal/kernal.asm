...
!if ULTIMATE_BUILD {
    ...
    !byte $00                                ; $e4ab
    !byte $81                                ; $e4ac padding before $e4ad routine
    pha                                      ; $e4ad
    jsr $ffc9                                ; $e4ae
    tax                                      ; $e4b1
    pla                                      ; $e4b2
...
; =============================================================================
    ...
    sta $028b                                ; $e52e
    sta $0288                                ; $e531
    lda #$0e                                 ; $e534  text colour = light blue (factory; DD2 used $01 white)
    sta $0286                                ; $e536
    lda #$0a                                 ; $e539
    sta $0289                                ; $e53b
    lda #$0c                                 ; $e53e
    ...
    sta $d5                                  ; $e58c
    jmp $ea24                                ; $e58e
    !byte $18,$60,$ea   ; $e591-$e593 STUB @ handler: clc/rts/nop (#4)
    jmp $e101                                ; $e594
    jmp $f894                                ; $e597
    jsr $e5a0                                ; $e59a
    jmp $e566                                ; $e59d
...
; --- Receive loop: get 8 bits from $DD00 bit 7 ---
    ...
    sta $dd00                                ; $ed8b
; JD_ENABLE: 0 = pristine working ROM (this splice + hole are byte-identical to the
; DolphinDOS base); 1 = JiffyDOS detection probe active. The probe is PROVEN to
...
!if JD_ENABLE {
    ...
    dec $a5        ; Decrement bit counter   ; $ed8e
    bne $ed66      ; Loop until 8 bits sent   ; $ed90
}
    lda #$04                                 ; $ed92
    sta $dc07                                ; $ed94
    lda #$19                                 ; $ed97
...
!if JD_ENABLE {
; =============================================================================
; JD_LOOPCHK - JiffyDOS in-band detection probe (spliced from $ed8e)
; Fires only when about to clock the LAST (8th) command bit ($a5==1) with ATN
; asserted. At that instant bit 7's edge ($ed84-$ed8b) left the bus in {DATA OUT
; released, CLK OUT held} - the open-roms JiffyDOS probe precondition. We watch
; DATA IN: a JiffyDOS drive pulls it low within ~400us; a stock drive stays
...
JD_LOOPCHK:
    dec $a5              ; (moved from $ed8e) bit counter 8..0
    beq JD_TOACK         ; ==0: all 8 bits sent -> post-byte ACK
    lda $a5
    cmp #$01             ; about to clock the LAST bit?
    bne JD_CONT          ; no -> ordinary loop iteration
    lda $dd00
...
JD_HIT:
    ...
    sta $9e
    ldx #JD_WINDOW       ; wait for the drive to release DATA again
JD_HR:
    lda $dd00
    and #$80
...
JT_RG:
    ...
    cpx #$fa                                 ; $fe9c
    bcs $feb4                                ; $fe9e
    !byte $6c,$02,$a0   ; $fea0-$fea2 RESTORE/NMI monitor entry -> jmp ($a002) warm start (#3)
    rol                                    ; $fea3
    bcs $feb5                                ; $fea4
    bmi $feb5                                ; $fea6
    ldx #$17                                 ; $fea8
...
; =============================================================================
    ...
    and #$10                                 ; $ff51
    beq $ff58                                ; $ff53
    !byte $6c,$16,$03   ; $ff55-$ff57 BRK dispatch: jmp $f01d -> jmp ($0316) stock (#3)
    jmp ($0314)                              ; $ff58
    jsr $e518                                ; $ff5b
    lda $d012                                ; $ff5e
    bne $ff5e                                ; $ff61
